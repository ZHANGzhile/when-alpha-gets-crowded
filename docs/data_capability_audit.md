...
第7–8项已选择Tushare Pro作为待验收接入源：`index_weight`返回月度指数成分及权重，脚本将最近
一个已知月度锚点按个股收益漂移到每个周度决策日；`stk_limit`返回每日涨停价和跌停价，官方文档
称其约在交易日9点更新，脚本按9:00可用、9:30执行的保守时序与BaoStock开盘价、停牌状态合成
方向性约束。两接口均要求账户至少2000积分，当前环境尚未提供`TUSHARE_TOKEN`，因此这是已实现但
尚未完成真实验收的软件路径。依据见[Tushare指数成分和权重接口](https://tushare.pro/document/2?doc_id=96)
与[每日涨跌停价格接口](https://tushare.pro/document/2?doc_id=183)。

复制篮子不能仅凭逐日权重和为1而放行。控制器现包含`active_weight_benchmark=0`的纯基准策略，按
同一开盘成交限制和5/10/20bp成本路径执行，并输出独立质量报告。真实数据揭盲前预设放行线为：
年化毛Tracking Error不超过1%、年化净Tracking Error不超过2%、毛年化主动收益绝对值不超过1%。
任何因子副本或成本情景失败都会保留诊断文件并停止P7，防止把基准复制误差归因于控制器。
